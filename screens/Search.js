...
export default class Home extends Component {
    ...
    getData = (text) => {
        const dataNew = lists.filter(image => {
            const nameData = image.name.toUpperCase()
                ...
                <View>
                    <FlatList
                        ...
                        renderItem={({item, index})=> {
                            return (
                                <Image source={{uri: item.image}} style={[styles.image, index%3 !==0 ? {marginLeft: 4} : null, {marginBottom: 4}]}/>
                            )
                        }}
                />
                </View>
